...
void utkmmap(pagetable_t ukpagetable, pagetable_t upagetable, uint64 usersz, uint64 uszend)
{
  ...
  // if (uszend >= PLIC)
  //   panic("utkmmap");
  // // 我们只对要映射的页表的PGROUNDUP页面进行映射，这样可以避免remap。
  // // 可以参考uvmalloc的代码获得理解
  // usersz = PGROUNDUP(usersz);
  // printf("first usersz: %d\n", usersz);

  // for (; usersz < uszend; usersz += PGSIZE)
  // {
  //   pte = walk(upagetable, usersz, 0);
  //   if (pte == 0)
  //   {
  //     printf("usersz: %d, uszend: %d \n", usersz, uszend);
  //     panic("utkmmap: not mapped");
  //   }
  //   pa = PTE2PA(*pte);
  //   ukvmmap(ukpagetable, usersz, pa, PGSIZE, PTE_V | PTE_R | PTE_W | PTE_X);
  // }
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(usersz); i < uszend; i += PGSIZE){
    if((pte = walk(ukpagetable, i, 0)) == 0)
    ...
    ukvmmap(ukpagetable, i, pa, PGSIZE, flags);
  }
}
...
void utkdealloc(pagetable_t ukpagetable, uint64 oldsz, uint64 newsz)
{
  ...
  // if((va % PGSIZE) != 0)
  //   panic("utkmunmap: not aligned");
  
  // for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
  //   if((pte = walk(ukpagetable, a, 0)) == 0)
  //     panic("utkmunmap: walk");
  //   if((*pte & PTE_V) == 0)
  //     panic("utkmunmap: not mapped");
  //   if(PTE_FLAGS(*pte) == PTE_V)
  //     panic("utkmunmap: not a leaf");
  //   *pte = 0;
  // }
  if(newsz >= oldsz)
    return;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    ...
    uvmunmap(ukpagetable, PGROUNDUP(newsz), npages, 0);
  }
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 2; level > 0; level--) {
    ...
    if(*pte & PTE_V) {
      ...
      // CMT: 每一级页表中，单个页表大小就是一个页的大小，4096
...
// 用于内核页表的寻址
uint64
kwalkaddr(struct proc *p, uint64 va)
{
  pagetable_t pagetable = p->ukpagetable;
  pte_t *pte;
  uint64 pa;
...
// 它这个映射相当于是把从va开始的一个page大小的虚拟地址的映射关系都建立起来
// 所以说就有可能要建立两次
// 需要注意一个虚拟地址不能被映射到两个物理地址中去
int
...
// 一个用于打印页表内容的函数
void
vmprint_inside(pagetable_t pagetable, int level)
{
  // there are 2^9 = 512 PTEs in a page table.
  ...
  for(int i = 0; i < 512; i++){
    ...
    if(pte & PTE_V){
      ...
      uint64 child = PTE2PA(pte);
      vmprint_inside((pagetable_t)child, level - 1);
    }
  }
}
